defmodule GameTest do
  ...
  test "Can move cards between tableaus" do
    ...
    assert length(moves) == 3
    # Ace of diams to empty foundation
    assert moves == [
             {:tableau, 3, :foundation, 0, {:diams, 1}},
             # 2 of spades to 3 of hearts
             {:tableau, 6, :tableau, 4, {:spades, 2}},
             # Ace of diams on 2 of spades
             {:tableau, 3, :tableau, 6, {:diams, 1}}
           ]
  end
  ...
  test "Detect tableau mismatches" do
    ...
    assert errors == [
             {:tableau_mismatch, 0, Cards.new(:diams, 3), Cards.new(:diams, 1)},
