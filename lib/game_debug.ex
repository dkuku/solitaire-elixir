defmodule Solitaire.Game.Debug do
  ...
  def validate(game) do
    ...
    validate_tableaus(tableaus, 0) ++
      validate_foundations(foundations, 0) ++ validate_foundation_starts_with_ace(foundations, 0)
  end

  @spec validate_foundation_starts_with_ace([Foundation.t()], non_neg_integer) :: [
          validation_error
        ]
  defp validate_foundation_starts_with_ace([], _index), do: []

  defp validate_foundation_starts_with_ace([foundation | tl], index) do
    starts_with_ace(Foundation.up(foundation), index) ++
      validate_foundation_starts_with_ace(tl, index + 1)
  end
  ...
  defp validate_tableaus([hd | tl], index),
    do: validate_tableau(Tableau.up(hd), index) ++ validate_tableaus(tl, index + 1)
